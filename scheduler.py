...
from worker import extractor_handler, spider_handler
from config import *
...
def fork_process(x):
    ...
    for i in xrange(x):
        detail = {}
        p = Process(target = request_worker, args = (spider_handler, "spider :fetch"))
        p.start()
        detail['obj'] = p
...
def spawn_worker():
    parent_id = os.getpid()
    p = Process(target = request_worker, args = (extractor_handler, "spider :extractor"))
    p.start()
    detail = {}
    detail['obj'] = p
    detail['is_running'] = True
